...
require_node() {
  ...
  if [[ -z "$docker_image" ]] && \
    ...
    echo "Non-Docker MCP mode requires either a global mcp-server-azuredevops binary, npx, or npm. Install npm with Node.js 20+ or install @azure-devops/mcp globally." >&2
    exit 1
  fi
}
