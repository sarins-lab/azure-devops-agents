...
Codex does not load Claude slash command files, so this adapter treats `/plan-epic`, `/plan-feature`, and `/plan-story` as instruction routes and relies on the canonical workflows appended from `shared/workflows/`.
